...
def find_header_row(df_head):
    ...
    for i, row in df_head.iterrows():
        # Łączymy wszystkie komórki w wierszu w jeden tekst
    ...
    return 0 # Domyślnie 0 (jeśli nie znajdziemy nagłówka, zakładamy że go nie ma)
...
def is_price(val):
    ...
    # Proste wyrażenie regularne sprawdzające, czy to liczba (może być dziesiętna)
...
def parse_import_file(self, import_job_id: int, filepath: str):
    ...
    try:
        ...
        try:
            # Wczytujemy jako str, aby uniknąć problemów z konwersją EAN
        ...
        # Krok 3: Przeładuj DF z poprawnym nagłówkiem (lub bez, jeśli nie znaleziono)
        try:
            ...
            # Jeśli ponowne wczytanie się nie uda (np. header_row_index=0), wróć do pierwotnego DF
            df = pd.read_excel(filepath, header=None, dtype=str) if filepath.lower().endswith(".xlsx") else pd.read_csv(filepath, header=None, dtype=str)
        ...
        if not all([ean_col, name_col, price_col]):
            ...
            # --- POPRAWKA (KROK 30): Tworzymy kopię wycinka, aby uniknąć SettingWithCopyWarning ---
        ...
        # --- POPRAWKA (KROK 30): Użyj .loc, aby uniknąć SettingWithCopyWarning ---
        # Sprawdzamy, czy df nie jest już kopią (jeśli nie był w Kroku 5)
        if not df.is_copy:
            df = df.copy()
        ...
        df.loc[:, "price_norm"] = pd.to_numeric(
            # Usuwa też znaki walut, jeśli jakieś są
        ...
        for _, row in df.iterrows():
            ...
            p = models.ProductInput(
                ...
                currency=job.meta.get("currency", "PLN"), # Na razie ignorujemy walutę z pliku
                status="pending",
            )
            db.add(p)
        ...
        raise e # Rzuć błąd, aby Celery oznaczył zadanie jako FAILURE
    finally:
        db.close()
...
def fetch_allegro_data(self, product_input_id: int, ean: str):
    ...
    try:
        ...
        if cache and cache.fetched_at > ttl_limit:
            # Użyj cache TYLKO jeśli NIE był to błąd
            if cache.source != 'failed' and cache.source != 'error':
                ...
                db.commit()
                db.close()
                return # <-- Wyjdź tylko jeśli mamy Dobre dane z cache
            
            # Jeśli cache.source == 'failed', ignorujemy cache i przechodzimy do scrapingu
        
        # --- KONIEC POPRAWKI ---
        
        result = scraper_fetch(ean) # Uruchom scraper (Krok 29)
